...
export default async function CategoryPage({
  ...
  try {
    const res = await (apiClient as any).categories[":id"].content.$get({
      param: { id },
    });
    if (res.ok) {
    ...
    console.error("Failed to fetch category content:", e);
  }
  ...
  return (
    <main className="min-h-screen bg-black text-white pb-24">
      ...
      <div className="p-4 md:p-12">
        <h1 className="text-xl md:text-4xl font-black mb-2 uppercase tracking-tighter">
          {categoryData.name}
        </h1>
        {categoryData.description && (
          <p className="text-gray-400 text-sm md:text-lg mb-8 md:mb-12 max-w-2xl">
            {categoryData.description}
          </p>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8">
          {categoryData.content.length > 0 ? (
            categoryData.content.map((item: any) => (
              <Link
                key={item.id}
                href={`/watch/${item.id}`}
